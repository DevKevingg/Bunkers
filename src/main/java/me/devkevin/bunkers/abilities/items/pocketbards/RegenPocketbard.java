...
public class RegenPocketbard extends Ability {
	...
	public List<String> getLore() {
		return Bunkers.getPlugin().getAbilityYML().gc().getStringList("Pocketbard.Regen.Lore");
	}

	@Override
	public String displayName() {
		return Bunkers.getPlugin().getAbilityYML().gc().getString("Pocketbard.Regen.Name");
	}
	...
	public Material material() {
		return Material.valueOf(Bunkers.getPlugin().getAbilityYML().gc().getString("Pocketbard.Regen.Material"));
	}
	@Override
		...
		if (item().isSimilar(event.getItem())) {
			...
			for (String message : Bunkers.getPlugin().getAbilityYML().gc().getStringList("Messages.Pocketbard.Used-Message")) {
				player.sendMessage(CC.chat(message
						.replaceAll("%heart%", "\u2764")
						.replaceAll("%pocketbard%", "&4Regen")
						.replaceAll("%time%", String.valueOf(time))
						.replaceAll("%effect%", "Regen " + power)));
			}
			PotionEffectType potType = PotionEffectType.REGENERATION;
			int rad = config.getInt("Pocketbard.Regen.Radius");
